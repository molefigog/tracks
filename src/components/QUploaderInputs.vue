<template>
    ...
    <q-uploader ref="uploader" :flat="flat" :max-files="fileLimit" :max-file-size="maxFileSizeInKB" :bordered="bordered"
        :accept="accept" :factory="factoryFn" :label="getLabel" auto-upload hide-upload-btn :multiple="multiple"
        @uploaded="uploadComplete" @removed="removeFile" @rejected="onRejected">
            ...
            <q-list separator>
                <q-item v-for="file in scope.files" :key="file.__key">
                    ...
                    <q-item-section top side>
                        <q-btn class="gt-xs" size="12px" color="red" flat dense round :icon="deleteIcon"
                ...
                <div class="row">
                    <template v-for="file in getUploadedFiles" :key="file.path">
                        <q-chip size="lg" square removable @remove="removeUploadedFile(file)" v-if="file.isImage">
                            <q-avatar><image-viewer-p :src="file.path" :image-size="file.size"></image-viewer-p></q-avatar>
                        </q-chip>
                        <q-chip square :icon="icon" :label="file.shortName" removable @remove="removeUploadedFile(file)"
                            clickable @click="openFile(file)" v-else></q-chip>
                    </template>
                </div>
            </template>
        </template>
...
export default {
    ...
    computed: {
        ...
        getUploadedFiles1() {
            let files = [];
            this.uploadedFilePaths.forEach(path => {
                // Append '/storage/' to the beginning of the path
                path = 'storage/' + path;
        ...
        getUploadedFiles3() {
            let files = [];
            const imgExt = ['jpg', 'png', 'gif', 'jpeg', 'bmp'];

            this.uploadedFilePaths.forEach(path => {
                // Append '/storage/avatars' to the beginning of the path
                path = 'storage/avatars/' + path;
                ...
                let isImage = imgExt.includes(ext);

                // Determine size
                let size = path.includes("temp/") ? "" : "small";
